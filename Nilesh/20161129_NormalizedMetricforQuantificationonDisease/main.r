...
for (i in 1:nrow(pouch.lot.number.df)){
#for (i in 1:20){
  temp <- get(paste0("internal.cp.pouch.lot.number.df.", pouch.lot.number.df$PouchLotNumber[i], sep = ""))
  if(nrow(temp) > 0){
    extracted.internal.cp.controls.df <- assign(paste0("internal.cp.pouch.lot.number.df.", pouch.lot.number.df$PouchLotNumber[i], sep = ""), temp)
    ...
    customer.cp.controls.df <- cust.cp.runs.df[cust.cp.runs.df$ResultType == 'control' & cust.cp.runs.df$PouchLotNumber == cust.cp.runs.df$PouchLotNumber[i], c('PouchSerialNumber', 'PouchLotNumber', 'AssayName', 'Cp')]
    ...
    g6 <- ggplot() + ylab(paste("Cp", "\u00B1", "std.dev.")) + ggtitle(paste('Customer control Cp data for ',  pouch.lot.number.df$PouchLotNumber[i], sep = ""))

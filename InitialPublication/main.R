...
if(TRUE) {
  ...
  workDir <-'G:/Departments/PostMarket/DataScienceGroup/Data Science Products/InProcess/Aimie/20160721_TrendExternal2016/Publication_Update/PublicationRefresh/'
  setwd(workDir)
  ...
  # test the dateToEpiweek function from the EpiWeek library
  # calendar.df$Year <- dateToEpiweek(calendar.df$Date, format='%Y-%m-%d')$year
  # calendar.df$Week <- dateToEpiweek(calendar.df$Date, format='%Y-%m-%d')$weekno
...
if(TRUE) {
  ...
  p.PercentDetectionTrend_Grouped <- ggplot(prevalence.nat.families[with(prevalence.nat.families, order(Name, decreasing=TRUE)),], aes(x=YearWeek)) + geom_area(aes(y=Prevalence, fill=Name, group=Name, order=Name), stat='identity', position='stack') + scale_fill_manual(values=bug.family.Pal, name='') + scale_x_discrete(breaks=dateBreaks, labels=dateLabels) + scale_y_continuous(label=percent, limits=c(0,0.8)) + theme(text=element_text(size=22, face='bold'), axis.text=element_text(size=22, color='black', face='bold'), axis.text.x=element_text(angle=90, hjust=1), legend.position='bottom', panel.background=element_rect(color='white', fill='white'), axis.ticks.x=element_blank()) + labs(title='Percent Detection of Organism Families in Trend Population', y='Percent Detection', x='Date')
}
...
if(TRUE) {
  ...
  dateBreaksAlt2 <- c('2015-40','2016-01','2016-14','2016-27','2016-40')
  dateLabelsAlt2 <- c('-','Jan-2016','-','--','-')
  ...
  # - RSV---------------------------------------------------------------------------------------------------------
  if(TRUE) {
    p1 <- ggplot(subset(prevalence.nat.individual.wrap[with(prevalence.nat.individual.wrap, order(ShortName, decreasing=TRUE)),], Bug %in% rsv), aes(x=YearWeek)) + geom_area(aes(y=Prevalence, fill=ShortName, group=ShortName, order=ShortName), stat='identity', position='stack') + scale_fill_manual(values=bug.individual.Pal, name='') + scale_x_discrete(breaks = dateBreaks, labels = dateLabels) + scale_y_continuous(label=percent, breaks=c(0, 0.06, 0.12, 0.18, 0.24, 0.30)) + theme(text=element_text(size=22, face='bold'), axis.text=element_text(size=22, color='black', face='bold'), axis.text.x=element_text(angle=90, hjust=1), legend.position='bottom', panel.background=element_rect(color='transparent', fill='white'), panel.grid=element_blank(), axis.ticks.x=element_blank())  + labs(title='Percent Detection of RSV in Trend Population with ILI Overlay', y='Percent Detection of Organism', x='Date') 
    p2 <- ggplot(subset(prevalence.nat.individual.wrap[with(prevalence.nat.individual.wrap, order(ShortName, decreasing=TRUE)),], Bug %in% rsv), aes(x=YearWeek, y=6*Rate, group=1)) + geom_line(color='black', lwd=2) + geom_line(aes(x=YearWeek, y=6*NormalizedBurn/100, group=2), subset(ili.burn.nat, as.character(ili.burn.nat$YearWeek) >='2014-01'), color='red', lwd=2) + scale_x_discrete(breaks = dateBreaks, labels = dateLabels) + scale_y_continuous(limits=c(0,6*max(prevalence.nat.individual.wrap$Rate)), breaks=c(0, 0.06, 0.12, 0.18, 0.24, 0.30), labels=c('0%','1%','2%','3%','4%','5%')) + theme(text=element_text(size=22, face='bold'), axis.text=element_text(size=22, color='black', face='bold'), axis.text.x=element_text(angle=90, hjust=1), legend.position='bottom', panel.background=element_rect(fill='transparent', color='transparent'), panel.grid=element_blank(), axis.ticks.x=element_blank()) + labs(y='ILI (black), FilmArray Test Utilization (red)')

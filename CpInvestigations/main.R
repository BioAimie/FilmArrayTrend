...
for(i in 1:length(panels)) {
  ...
  for(k in 1:length(assays)) {
    ...
    if(choose.query == 'RP') {
      ...
      temp.file <- paste(imgDir, choose.query, '_', choose.sites, '_', gsub('\\/','-', assays[k]), '_TimeSeriesCps.png', sep='')
    } else {
      
      temp.file <- paste(imgDir, choose.query, '_', gsub('\\/','-', assays[k]), '_TimeSeriesCps.png', sep='') 
    }
  ...
  for(k in 1:length(assays)) {
    ...
    temp.file <- paste(imgDir, choose.query, '_', gsub('\\/','-', assays[k]), '_DistributionCps.png', sep='')
    png(temp.file, width = 1400, height = 800)
    print(temp.plot)
    dev.off()
  }
  ...
  for(k in 1:length(targets)) {
    ...
    if(length(as.character(unique(temp.dat$TargetTriggerAssay)))==1) { 
      ...
      temp.file <- paste(imgDir, choose.query, '_', gsub('\\/','-', targets[k]), '_DistributionCpsByTarget.png', sep='')
      png(temp.file, width = 1400, height = 800)
      print(temp.plot)
      dev.off()
    }
  ...
  if(choose.query=='BCID') {
    ...
    for(k in 1:length(genes)) {
      ...
      temp.file <- paste(imgDir, choose.query, '_', gsub('\\/','-', genes[k]), '_DistributionDeltaCps.png', sep='')
      png(temp.file, width = 1400, height = 800)
      print(temp.plot)
      dev.off()
    }
